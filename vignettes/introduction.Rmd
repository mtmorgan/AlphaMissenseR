...
The return value `db` is a DuckDB database. The database contains
tables for each key downloaded. View available tables with

```{r am_data-duckdb}
db_tables(db)
```

Create a [dplyr][] / [dbplyr][] tibble of the table of interest.
...
It is straight-forward and fast to summarize the data, e.g., the
...
We will work with the hg38 annotations from AlphaMissense.
...
there are many name conflicts between functions in different packages,
...
cannonical transcript of a particular gene.
...
interset (with columns `#CHROM`, `start`, `end`), and the temporary
table to contain the results. A range join is like a standard database
join, expect that the contstraints can be relations, in our case that
`POS >= start` and `POS <= end` for each range of interset;
...
It is perhaps instructive to review the range join template as a
source of inspiration for other computations that might be of interest.
...
As always, remember to disconnect and shutdown the DuckDB connection.
